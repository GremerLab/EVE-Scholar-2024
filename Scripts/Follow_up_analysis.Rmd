...
```{r}
count_sum <- week1 %>%
  mutate(count = rowSums(select(.,8:14), na.rm=TRUE),
...
```{r}
ggplot(wk1_avg, aes(x=Days_to_Germ)) +
...
```{r}
ggplot(wk1_long, aes(x=Treatment, y=Germ_rate, color=Treatment)) +
  geom_boxplot()+
  theme_bw()+
